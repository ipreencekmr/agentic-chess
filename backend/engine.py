...
class StockfishService:
    ...
    def get_top_moves(self, fen: str, depth: int = 12, k: int = 3):
        ...
        for entry in info:
            move = entry["pv"][0].uci()
            # Use a large number for mate score to distinguish from centipawn
    ...
    def evaluate(self, fen: str, depth: int = 12):
        # Evaluate the given FEN at the specified depth
        board = chess.Board(fen)
    ...
    def close(self):
        # Properly close the Stockfish engine process
    ...
    @classmethod
    def shutdown(cls):
        # Shutdown the singleton instance and close the engine
    ...
    @classmethod
    def getInstance(cls):
        # Get or create the singleton instance of StockfishService
