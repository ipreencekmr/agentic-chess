...
def is_openai_ready() -> bool:
    """Check if OpenAI API key is set and openai package is available."""
...
def _capture_priority_move(board: chess.Board, legal_moves: list[chess.Move]) -> chess.Move:
    """Choose a capture move if available, otherwise pick randomly from legal moves."""
...
def _get_stockfish_move(board: chess.Board, legal_moves: list[chess.Move], skill_level: int, depth: int) -> tuple[str | None, str | None]:
    ...
    If Stockfish fails or returns an illegal move, fallback to capture-priority heuristic.
    """
    try:
...
def _get_ai_agent_move(board: chess.Board, legal_moves: list[chess.Move], ai_model: str) -> tuple[str | None, str | None]:
    """
    Get a move from the AI agent using OpenAI.
...
def choose_ai_move(board: chess.Board, difficulty: str, ai_model: str) -> tuple[str | None, str | None]:
    """
    Choose an AI move based on the specified difficulty and model.
